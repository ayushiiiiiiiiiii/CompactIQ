    ...
    <div style={{ display: 'flex', height: '100vh', width: '100vw', backgroundColor: 'var(--bg-color)', color: 'var(--text-primary)', transition: 'all 0.3s' }}>
      <nav className="glass-panel" style={{ width: '260px', margin: '16px', padding: '24px', display: 'flex', flexDirection: 'column', background: 'var(--sidebar-bg)', border: '1px solid var(--glass-border)' }}>
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <li><Link to="/admin/kb-admin" className="nav-link" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><Database size={18} /> KB Admin</Link></li>
        </ul>
        
        <div style={{ marginTop: 'auto', padding: '16px', background: 'rgba(0,0,0,0.2)', borderRadius: '8px', color: '#ffffff', fontSize: '12px', opacity: 0.8 }}>
...
const ClientLayout = ({ theme, toggleTheme }) => {
  const { loadingStatus, phaseIndex } = React.useContext(AppContext);
  const { scanError } = useDeviceScan();
  ...
  if (scanError) {
      return (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', backgroundColor: 'var(--bg-color)' }}>
              <div style={{ textAlign: 'center', padding: '40px', backgroundColor: 'var(--card-bg)', borderRadius: '12px', border: '1px solid var(--error-border)', boxShadow: 'var(--shadow)' }}>
                  <div style={{ fontSize: '48px', marginBottom: '20px' }}>⚠️</div>
                  <h2 style={{ color: 'var(--error-text)', margin: '0 0 15px 0' }}>Validation Failed</h2>
                  <p style={{ color: 'var(--text-secondary)', margin: '0 0 25px 0' }}>{scanError}</p>
                  <button onClick={() => window.location.reload()} style={{ padding: '10px 20px', backgroundColor: 'var(--error-border)', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}>Retry Scan</button>
              </div>
          </div>
      );
  }
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', backgroundColor: 'var(--bg-color)' }}>
              {phaseIndex === 0 ? (
                  <div style={{ textAlign: 'center', animation: 'fadeIn 1s ease-in' }}>
                      ...
                      <p style={{ color: 'var(--text-primary)', fontSize: '20px', fontWeight: '500' }}>Endpoint Agent Initialization</p>
                      <p style={{ color: 'var(--text-secondary)', fontSize: '14px', marginTop: '20px' }}>Loading services...</p>
                      <style>{`@keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }`}</style>
                  </div>
              ) : (
                  <div className="glass-card" style={{ textAlign: 'center', padding: '40px', width: '450px' }}>
                      <div style={{ marginBottom: '25px' }}>
                          <div className="spinner" style={{ width: '48px', height: '48px', border: '4px solid var(--card-border)', borderTop: '4px solid #10b981', borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto' }}></div>
                          <style>{`@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }`}</style>
                      </div>
                      <h3 style={{ color: 'var(--text-primary)', margin: '0 0 20px 0', fontSize: '18px', fontWeight: '600' }}>{LOADING_PHASES[phaseIndex]}</h3>
                      <div style={{ width: '100%', backgroundColor: 'var(--card-border)', borderRadius: '6px', height: '8px', overflow: 'hidden' }}>
