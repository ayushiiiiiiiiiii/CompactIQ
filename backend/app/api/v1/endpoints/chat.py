...
async def chat_assistant(request: ChatRequest, db: AsyncSession = Depends(get_db)):
    if not client:
        return ChatResponse(answer="Error: Gemini API is not configured or failed to initialize.")
    ...
    if not device:
        return ChatResponse(answer="I couldn't find any compliance records for that device.")
