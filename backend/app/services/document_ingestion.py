...
# Additional compatibility documents can be added without changing
# application code.
...
async def load_and_ingest_seeds():
    ...
    if not os.path.exists(seed_dir):
        logger.warning(
            f"Startup Ingestion: Seed directory does not exist: {seed_dir}")
        os.makedirs(seed_dir)

    discovered_files = [f for f in os.listdir(
        seed_dir) if f.endswith((".pdf", ".txt"))]
    logger.info(
        f"Startup Ingestion: {
            len(discovered_files)} files discovered: {discovered_files}")
    ...
    for filename in discovered_files:
        ...
        logger.info(
            f"Startup Ingestion: File processed: {filename} | Rules extracted: {rules_extracted}")
        total_rules_inserted += rules_extracted
        files_processed += 1

    logger.info(
        f"Startup Ingestion Complete: {files_processed} total files processed, {total_rules_inserted} total rules inserted.")
    ...
    async with async_session() as session:
        ...
        logger.info(
            f"Database State post-ingestion: {doc_count} documents, {rule_count} rules.")

        if len(discovered_files) > 0 and rule_count == 0:
            logger.error(
                "Startup Ingestion Validation Failed: Compatibility documents are present, but ZERO rules exist in the database!")


async def ingest_document_file(
        filepath: str, filename: str) -> tuple[int, int]:
    async with async_session() as session:
...
class ExtractedRule(BaseModel):
    source_component_type: str = Field(
        description="The component type requiring or conflicting with something (e.g., OS, SecurityAgent, BIOS)")
    source_version: str = Field(
        description="The version of the source component, or * if not specified")
    target_component_type: str = Field(
        description="The component type being required or conflicted with")
    target_min_version: str | None = Field(
        default=None, description="Minimum required version, if applicable")
    target_max_version: str | None = Field(
        default=None, description="Maximum allowed version, if applicable")
    incompatible_version: str | None = Field(
        default=None,
        description="Specific version that is incompatible, if applicable")
    rule_type: str = Field(
        description="Type of relationship: REQUIRES, DEPENDS_ON, SUPPORTED_ON, CONFLICTS_WITH")
    reason: str = Field(
        description="Explanation of why this rule exists based on the document")
...
async def extract_rules_from_text(text: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if len(text) > 50000:
            print(
                "Warning: Document text exceeds 50,000 characters. Truncating to prevent token limits.")
            text = text[:50000]
...
---"""
        ...
        # Ensure we return a list of dicts that match what the DB insertion
        # expects
